...
5. CORE INVARIANTS: List all empirical benchmarks, latency bounds, and architectural guarantees.`;
  ...
  public async evaluateAndAuditSlide(
    ...
    try {
      const client = this.getClient();
      const prompt = `You are the Keynox Quality Evaluator & Academic Editor (Auditor Tier).
Audit this slide HTML generated by the NVIDIA model for topic "${topic}".
...
3. ZERO EMOJIS: Strict academic restraint—replace any emojis with clean text or remove them.
...
- ONLY if repairs were required, output the repaired <section class="slide ...">...</section> block directly with no conversational markdown or backticks.`;
      ...
      const response = await client.chat.completions.create({
        ...
        messages: [
          {
            role: "system",
            content: "You are an elite academic presentation auditor and KaTeX/HTML validator. Be concise.",
          },
